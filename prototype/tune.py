...
def train(awl_lr : float,  batch_size : int, num_epochs : int):
    ...
    for epoch in range(num_epochs):
        ...
        torch.save(aplnet.state_dict(),model_save_path+str(epoch)+'.pth')
        torch.save(awl.state_dict(),model_save_path+str(epoch)+'_s.pth')
        ...
        print(awl.params)
        awl_p.append(awl.params.detach().numpy().tolist()[0]/awl.params.detach().numpy().tolist()[1])
        ...
        if a > best_test_act:
            ...
            best_epoc = epoch
            torch.save(aplnet.state_dict(), "./weights/best_model_awllr"+str(awl_lr)+"_batchsize_"+str(batch_size)+".pth")
        ...
        print(com_act,com_loc,best_epoc,best_test_act)
    ...
    f = open('res_batchsize.txt','a')
    f.write(str(com_act)+'\t'+str(com_loc)+'\t'+str(best_epoc)+'\t'+str(best_test_act))
    f.write('\n')
    f.close()

@hydra.main(version_base=None, config_path="conf", config_name="config")
def tune_parameter(cfg: DictConfig) ->None:
    print(OmegaConf.to_yaml(cfg))
    awl_lr = cfg.train_setting.awl_lr
    batch_size = cfg.train_setting.batch_size
    num_epochs = cfg.train_setting.num_epochs
    train(awl_lr,batch_size,num_epochs)
